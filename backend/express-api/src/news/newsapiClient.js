...
class NewsAPIClient {
     ...
     * @param {number} pageSize - Number of articles per request (default: 100, max: 100)
     * @returns {Promise<Array>} Array of normalized articles
     */
    async search(name, riding = null, windowDays = 7, pageSize = 100) {
        try {
            // Check rate limit
            this.checkRateLimit();
            ...
            console.log(`NewsAPI search: ${query}, ${windowDays} days, ${pageSize} articles`);
            ...
            if (response.data.status === 'ok') {
                return this.normalizeArticles(response.data.articles, name, riding);
            } else {
    ...
    checkRateLimit() {
        ...
        if (this.rateLimit.requestsUsed >= this.rateLimit.requestsPerDay) {
            throw new Error(`NewsAPI rate limit exceeded: ${this.rateLimit.requestsPerDay} requests per day`);
        }
    }
