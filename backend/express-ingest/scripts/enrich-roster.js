...
/**
 * Roster Enrichment Script
 * Enriches the Alberta roster data with district information from the Represent API
 */
...
const OUTPUT_FILE = 'data/officials.json';
...
/**
 * Save enriched data to Azure Blob Storage
 */
async function saveEnrichedData(roster) {
  try {
    console.log(`[SAVE] Saving enriched data to Azure Blob Storage...`);
    ...
    // Use BlobServiceClient directly
    ...
    // Upload the data
    const blockBlobClient = containerClient.getBlockBlobClient(OUTPUT_FILE);
    ...
    console.log(`[SAVE] ✓ Successfully saved enriched data to ${OUTPUT_FILE}`);
    return enrichedData;
